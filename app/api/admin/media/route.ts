...
function hasValidSignature(
  ...
  if (contentType === "image/gif")
    return hasBytes(bytes, [0x47, 0x49, 0x46, 0x38]);
  return (
...
export async function POST(request: Request) {
  ...
  const metadata = imageUploadMetadataSchema.safeParse({
  ...
  if (entry.size < 1 || entry.size > MAX_IMAGE_BYTES) {
    return NextResponse.json(
      { message: "Ukuran gambar maksimal 30 MB." },
      { status: 400 },
    );
  }
  ...
  if (!hasValidSignature(bytes, contentType)) {
    return NextResponse.json(
      { message: "Isi file tidak cocok dengan tipe gambar." },
      { status: 400 },
    );
  }

  const { slot } = metadata.data;
  const previous = await query<{ object_key: string }>(
    "SELECT object_key FROM invitation_media WHERE slot = $1",
    [slot],
  );
  const objectKey = `invitation-media/${slot}/${crypto.randomUUID()}.${extensions[contentType]}`;

  try {
    const publicUrl = await putImage({
      key: objectKey,
      body: bytes,
      contentType,
    ...
    try {
      const result = await query<InvitationMedia>(
        ...
        [
          ...
          objectKey,
          publicUrl,
          sanitizeOriginalName(entry.name),
          contentType,
          entry.size,
        ],
      );
      ...
      if (oldKey && oldKey !== objectKey) {
        await deleteImage(oldKey).catch((cleanupError) =>
          console.error("Old R2 image cleanup failed", cleanupError),
        );
      }
    ...
    return NextResponse.json(
      {
        message:
          "Gambar belum dapat disimpan. Periksa konfigurasi R2 dan database.",
      },
      { status: 502 },
    );
  }
...
export async function PATCH(request: Request) {
  ...
  if (!result.rows[0]) {
    return NextResponse.json(
      { message: "Upload foto pada slot ini sebelum mengatur tampilannya." },
      { status: 404 },
    );
  }
